...
the weekly return of Apple stock is the predicted variable.
We will try to understand what affects APPLE stock price and incorporate as much
...
For this case study, other than the historical data of APPLE, the independent variables
used are the following correlated assets:
 -> Stocks: MSFT and AMZN
...
return_period = 5
Y = np.log(df_stock_data.loc[:,('Close','AAPL')]).diff(return_period).shift(-return_period)
Y.name = Y.name[-1]+'_pred'

X1 = np.log(df_stock_data.loc[:, ('Close', ('MSFT', 'AMZN'))]).diff(return_period).dropna()
X1.columns = X1.columns.droplevel()
X2 = np.log(currency_data).diff(return_period).dropna()
X3 = np.log(idx_data).diff(return_period).dropna()

X4 = pd.concat([np.log(df_stock_data.loc[:, ('Close', 'AAPL')]).diff(i) for i in [return_period, return_period*3,
...
pyplot.savefig('AAPL - Data Visualization - Histograms.png')
pyplot.show()

dataset.plot(kind='density', subplots=True, layout=(4,4), sharex=True, legend=True, fontsize=1, figsize=(15,15))
pyplot.savefig('AAPL - Data Visualization - Density.png')
pyplot.show()

#Box and Whisker Plots
dataset.plot(kind='box', subplots=True, layout=(4,4), sharex=False, sharey=False, figsize=(15,15))
pyplot.savefig('AAPL - Data Visualization - Box and Whisker Plots.png')
pyplot.show()
...
pyplot.savefig('AAPL - Correlation Matrix.png')
pyplot.show()
...
pyplot.savefig('AAPL - scatter-plot matrix.png')
plt.show()
...
fig.set_figheight(8)
fig.set_figwidth(15)
pyplot.savefig('AAPL - Trend and seasonality components.png')
pyplot.show()
...
Y = dataset["AAPL_pred"]
X = dataset.loc[:, dataset.columns != 'AAPL_pred']
...
pyplot.savefig('AAPL - Algorithm Comparison: Kfold results.png')
pyplot.show()
...
fig.suptitle('AAPL - Algorithms Comparison')
ax = fig.add_subplot(111)
...
ax.set_xticks(ind)
ax.set_xticklabels(names)
pyplot.savefig('AAPL - Algorithms Comparison.png')
pyplot.show()

# 5.3.2 - Time series–based models: ARIMA and LSTM
X_train_ARIMA = X_train.loc[:,['MSFT','AMZN', 'DEXJPUS', 'SP500', 'DJIA','VIXCLS']]
X_test_ARIMA = X_test.loc[:,['MSFT','AMZN', 'DEXJPUS', 'SP500', 'DJIA','VIXCLS']]
...
fig.suptitle('AAPL - Comparing the performance of various algorithms on the Train and Test Dataset')
ax = fig.add_subplot(111)
pyplot.bar(ind - width/2, train_results,  width=width, label='Train Error')
pyplot.bar(ind + width/2, test_results, width=width, label='Test Error')
fig.set_size_inches(15,8)
